...
- **Completion confetti + tab cross-off** — when every top-level item in a list becomes checked (≥1 item required), a confetti burst fires once (only on the incomplete → complete transition, not on every render). The list's tab shows a line-through and a small check mark. The effect clears automatically if any item is unchecked.
