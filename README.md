...
- **List-level due date** — optionally set a due date on the entire list via the **Due date** field in the New/Edit List form. Appears as a small pill (📅 Jun 20) in the list header, tinted with the list color. The pill has an inline × to clear it. The due date is additive (`list.dueDate: 'YYYY-MM-DD' | null`) — no schema or sync changes needed.
