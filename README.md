...
- **Each day box carries a `2/5` counter** in its top-right corner — how much of that day is behind you. It turns green only when the whole day is done. A cell shows the first three chips and hides the rest behind *"+2 more"*, so on a busy day this is the only complete count you get; the tooltip spells it out. A day holding an **exam** can never read as finished, which is honest rather than annoying: you don't tick off an exam, it happens to you, and a day claiming to be done with one still on it is the wrong kind of reassurance
- **What's left to do sorts to the top of every day**, ahead of the exam-then-assignment-then-task order. With finished work sorted in among the rest, a day where you'd already done the reading could show three ticked-off chips and hide the essay behind *"+2 more"* — and what's left is the question the grid is being asked. Exams are never done, so they keep their place at the front of the outstanding ones rather than being displaced
