...
- **Tags** — add free-form tags per note; tag chips appear above the list and filter it on click. Tags are case-insensitively de-duplicated, and a leading `#` is stripped, so `#chem`, `Chem` and `chem ` are all the same tag rather than three near-identical ones. Each note's own tags also render as pills on its row in the list (first 3, then a `+N` counter), tinted with that note's accent color.
  ...
  - **Furl notes into tag groups.** The **Group** toggle (beside the All / Starred filters) splits the list into one section per tag, ordered by name, with untagged notes last under **Untagged** — nothing can quietly vanish from the list for never having been tagged. Each section header furls it shut, leaving a single 28 px row where a screenful of notes was, and **Furl all** collapses everything at once. Headers are sticky, so scrolling a long group still tells you which tag you're inside.
  - A note with three tags appears under all three groups. That's the point of grouping by tag rather than filing each note in one folder — furling a group hides a *view* of a note, never the note. Pinned and starred notes still sort to the top **within** their group.
