...
- **Link a lab or studio to its class** — a section that meets at its own hour keeps its own meetings on the calendar, but its tasks, exams, notes and reminders all belong to the class, so there's one option to file work under instead of two
...
- **A long name on a short event stays readable** — a 30-minute block is one line tall, so its content is a race for horizontal room. The **name leads and the time trails**, which is the opposite of what it used to be. The time is the half you can already read off the row the block sits on; `9:00 AP C…` tells you nothing about which class it is, while `AP Chemistry Lab` tells you everything and you never needed the time
  - Below about **124 px of block width the time is dropped entirely** and the name is allowed a **second line**, which roughly doubles what a squeezed block can say. Two overlapping events split a day column in half and a day column is itself a seventh of the week view, so this is a real width, not a hypothetical one
  - Both are **container queries**, not JavaScript: what matters is the block's *rendered* width, which the React renderer never sees. The alternative is a measurement pass per event on every one of FullCalendar's frequent re-renders
  - The time disappears rather than shrinking. A half-clipped `9:0` is worse than no time at all — it reads as a bug, where an absent time reads as a design
- **Tall blocks clip by whole lines, never through one** — the title used to wrap freely and get sliced by the block's edge, cutting horizontally through a row of letters. The bottom half of a row of glyphs reads as broken rendering, not as truncation. The title now gets a `max-height` in whole line boxes (derived from duration, same proxy as the notes budget), so a long name simply stops at the end of a line
  - The budget is a **floor**, deliberately: rounding up buys a few more characters at the cost of reintroducing the half-line it exists to prevent. It caps at three lines — past that the block is a wall of text and the notes underneath, usually the room number, get pushed out of a block with room for them
  - The title never yields space; the notes below it do. The name is the thing you're scanning for
  - Anything that still overflows **fades out at the block's bottom edge** instead of ending on a hard slice. The fade is measured from the block, not from the text, so a title that fits is never touched — it lands on empty space
  - Hovering any event block shows its full name as a tooltip, whichever layout it drew in
...
- **Outstanding work sits above finished work in a cell.** A cell fits three chips, and items arrived in the order they were built — so a Thursday with two things ticked off and three still due could show two struck-through chips and hide one of the three under *"+1 more"*. The cell's job is to say what's **left**, so the thing it truncates should be the thing you've already dealt with. Exams are never done, so they keep their place among the outstanding ones rather than being displaced. Sorted rather than filtered: a day whose work is all done should still show it, and the counter needs the finished items to count
- **Each day carries a `done / total` counter** beside the date — *"1/3"* — so a glance says how much of a day is behind you, not just how much landed on it. It turns green when the day is clear, and the tooltip spells it out in words. Since the cell truncates at three chips, on a busy day this is the only complete count you get. **Exams are left out of both halves**: an exam isn't something you tick off, so counting it as outstanding would leave a day with one exam and one finished assignment reading *"1/2"* for ever, and a week of exams would look like a week of unfinished work. A day with nothing tickable on it shows no counter at all

  This one landed twice, independently, and the two versions disagreed only here: the other counted exams *so that* a day holding one could never read 3/3 — the exam is still ahead of you, and a day claiming to be finished is false reassurance. Reasonable, but it makes the number wrong rather than cautious: an exam's `done` is never true, so *"1/2"* can never become *"2/2"* however much you finish. The exam is already on the day as a chip, in the red overdue dot and in the workload tint, so leaving it out of a *work* counter loses no warning
...
- **Tags** — add free-form tags per note; tag chips appear above the list and filter it on click. Tags are case-insensitively de-duplicated, and a leading `#` is stripped, so `#chem`, `Chem` and `chem ` are all the same tag rather than three near-identical ones. Each note's own tags also render as pills on its row in the list (first 3, then a `+N` counter), tinted with that note's accent color.
  - **Re-using a tag is one click.** Under the editor's tag input sits a **Reuse** row of every tag already in use on another note that this one doesn't have yet; typing filters it, so it doubles as autocomplete. This is the whole reason the feature is worth having — a tag that is annoying to apply a second time splinters into `chem`, `chemistry` and `Chem 2`, and then no grouping or filtering can help you. There is no tag registry behind this: a tag exists exactly as long as some note wears it, which means there is never a list of dead tags to prune.
  - **Filter by several tags at once.** Chips are multi-select and show how many live notes carry each tag. Selecting more than one **widens** the list (notes tagged `chem` **or** `bio`) rather than narrowing it — "show me both subjects" is the question you actually ask of a chip row; the *and* reading is rarer and you can still get there by picking one tag and searching. A **Clear** button drops the whole selection.
  - **Furl a tag out of the way, without filtering anything.** Every tag chip carries a small chevron: click it and that tag's notes collapse into **one row** in the ordinary all-notes list — `chem ▸ 4 notes` — sitting exactly where those notes were. Everything else stays exactly as it was. Click the bundle row (or the chip's chevron again) to unfurl. This is the everyday use: furling is a way to *compact* the full list, not a filter that hides the rest of it. **Furl all** folds up every tag at once, leaving untagged notes in place.
    - The bundle sits at the position of the first note it swallows, so furling collapses a tag **in place** rather than reshuffling the list, and a stack of up to three colored spines on the row hints that it stands in for several notes. A blue dot appears on the bundle if the note currently open in the editor is inside it — otherwise furling the tag of the note you're reading looks like the note was closed.
    - **Asking to see a tag beats having furled it.** Selecting a furled tag as a filter shows its notes normally; clear the filter and it folds back up. The furl is remembered either way.
  - **Group mode** is the stronger version, for when you want the whole list filed rather than one tag tidied away. The **Group** toggle (right of the All / Starred filters) splits the list into one section per tag, ordered by name, with untagged notes last under **Untagged** — nothing can quietly vanish from the list for never having been tagged. Section headers furl the same way and are sticky, so scrolling a long group still tells you which tag you're inside.
  - A note with three tags appears under all three groups. That's the point of grouping by tag rather than filing each note in one folder — furling hides a *view* of a note, never the note. Pinned and starred notes still sort to the top **within** their group.
  - **Furling one tag folds away only what belongs to that tag alone.** A note tagged `chem` *and* `lab` stays in the flat list while `lab` is unfurled — furling one category shouldn't reach into a second one and take its notes with it. That note folds up once every tag it carries is furled, and is then shown once but counted in each bundle it belongs to, so every bundle reports exactly what it is standing in for.
  - **Grouping and furl state are device-local** (`lv-notes-grouped` / `lv-notes-furled` in `localStorage`), deliberately not synced. Furling a group on your phone to fit more on a small screen says nothing about how you want the list to look on a laptop — and syncing it would make notes appear to vanish on one device because of something you did on another. Trash is never grouped: it is a flat "what did I throw away" list, where a furled group would hide exactly what you came to look at.
...
### 🔗 Link a lab or studio to the class it belongs to

A lab, studio or recitation meets at a different hour than the lecture it belongs to, so the schedule has always needed two entries for it — which is correct on the calendar, where they really are two blocks at two times, and wrong everywhere the app asks *which class is this for*. **Organic Chemistry** and **Organic Chemistry Lab** meant two chips in the task filter, two entries in the category picker, two cards in My Classes, two places to keep one reminder rule, and a term's coursework split down the middle depending on which one you happened to pick.

So a section can now be declared **part of another class**. Open the lab from its edit form and pick the lecture under **Section of**. From then on the two are one class everywhere it matters:

- **One entry in every category picker and filter chip** — file work under *Organic Chemistry*, not under one of its two halves
- **One card in My Classes**, with each section's meeting pattern listed inside under **Sections** and its own **Edit** beside it. Its days, its room and its one-off cancellations are still its own; the coursework below is shared
- **One grade, one study-time total, one set of reminder rules.** A rule set on the class covers the lab's deadlines too — and only the class carries rules, so a lab that had its own from before is quietly ignored rather than sending a second notification for one deadline
- **One list of notes.** A note filed against either half is reachable from the class
- **The meetings stay separate.** Both blocks keep their own times, titles and room on the calendar — that difference is the entire reason the lab was a separate entry, and averaging it into one line would be a lie about when you have to be somewhere

**Nothing is rewritten.** Linking sets one field on the lab's row (`linkedToClassId`) and touches no task, event, note or exam. The tasks you had already filed under the lab keep exactly the category they had; everything that groups by class resolves the id to the parent when it *reads*. That is what makes the merge safe to try:

- **Unlinking puts both halves back exactly as they were** — including the tasks, which never moved. Rewriting `category` on every affected task would have been a bulk write across two synced tables that could not be undone once the two sets were indistinguishable
- **A bad link is one field to clear**, and there is no half-migrated state to recover from
- **The lab adopts the class's colour** when you link it. The two are one class everywhere else, and colour is what identifies a class in a week view before any text is read — leaving the lab green would make the calendar the one place still insisting they are two. It's an ordinary field afterwards, so you can change it back
- **The child names the parent, never the reverse.** A parent holding a list of its sections would need that list kept in step when a section is deleted on another device — the two-records-of-one-fact problem the rest of the app avoids. One field on one side cannot disagree with itself
- **A link whose target is gone is simply not a link.** Delete the lecture and the lab stands back on its own, with its own category and its own card — the behaviour that existed before links did. Same if the lecture is *disabled*: a switched-off class is out of the category picker, so resolving a live section onto it would leave that section's work with no home anywhere in the app
- **A chain is flattened.** Link the lab to the lecture and a review section to the lab, and all three resolve onto the lecture. The form says so rather than doing it silently: linking to something that is already a section tells you to link to the main class instead, and a class that already *has* sections is offered a sentence naming them instead of a picker

### 🗂 Task list headings — Overdue, Today, Upcoming, This Week…

The grouped task list used to jump straight from **Today** to **This Week**, and a week is a long time. In a normal school week that one heading collected most of the list, so the thing due tomorrow sat in the same undifferentiated pile as the thing due Friday — the list said *when* without ever saying *how soon*.

**Upcoming** splits the near half out: today + 1 through today + 3, the horizon you can still do something about. **This Week** keeps the rest of the seven days. The full order is **Overdue → Today → Upcoming → This Week → Next 2 Weeks → Further Out → No Date**, and each heading appears only when it has something in it.

- **The accent fades as the deadline recedes** — red, amber, the theme accent, a muted mix of the accent and grey, then plain grey. The top of the list reads as urgent without every heading shouting
- **Only "Upcoming" says its range** (`next 3 days`). "Upcoming" and "This Week" both mean *soon*, and without the hint the boundary between the two is a guess
- **The headings are a partition**, so each task is bucketed in a single pass rather than re-filtered once per heading. Every date lands in exactly one bucket by construction — a task can't fall between two headings and vanish
...
**Escape keeps what you typed.** Every one of these little text fields already committed on blur — clicking away from a half-typed step added it — but Escape threw the draft out, so the same typing survived or vanished depending on which way you happened to leave the field. Escape now commits, everywhere: the inline step composer on a task row, the step field and the rename-a-step field in the Add/Edit modal, and the item and subtask renames on custom lists.

- **In the modal it was losing more than the step.** Escape there didn't only discard the draft — nothing stopped it reaching the modal's own Escape handler, so it closed the entire form, and a modal that closes without saving takes the title, the date, the category and every other unsaved edit with it. Escape in a half-typed step field now adds the step and stays put
- **An empty field still closes the modal**, because there is nothing to lose and Escape means *close* everywhere else in the app. The change is only about not destroying work
- **The asymmetry is deliberate.** Escape-as-cancel is the usual convention, but the cost here isn't symmetrical: a step added by accident is one click to delete, and a step lost has no undo at all. When the two exits disagreed, the destructive one was the one you had to know about
- **Committing goes through blur rather than duplicating the add**, so there is still exactly one code path that appends a step — otherwise an Escape followed by the browser's own blur could add the same step twice
