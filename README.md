...
- **Boundaries are computed in local time.** They used to be built with `new Date(); d.setDate(d.getDate() + 7); d.toISOString().slice(0, 10)`, and `toISOString` converts to UTC first: west of Greenwich, every boundary rolled over to the next day in the evening and tasks quietly jumped a heading at dinner time. They now go through the same `localDate` helpers as the rest of the app, and take today as a parameter rather than reading the clock, so the list always agrees with the date the panel was handed
