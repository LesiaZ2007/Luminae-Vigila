...
describe('ClassesPanel — linked sections', () => {
  ...
  /* Asserted through exams because "Coming up" lists exams only now — it used to
     list every remaining meeting, which buried them. What matters here is unchanged:
     a linked section's own events surface on the parent class's card, so the lab's
     exam is not stranded on a card that no longer exists. */
  it("shows both sections' exams under Coming up", () => {
    renderLinked({
      canvasClassEvents: [
        meeting('2026-03-04', {
          title: 'Organic Chemistry midterm',
          extendedProps: { source: 'canvas-class', classId: 'chem', courseName: 'Organic Chemistry', isExam: true },
        }),
        meeting('2026-03-05', {
          id: 'canvascls_chem_lab_2026-03-05', title: 'Organic Chemistry Lab practical',
          start: '2026-03-05T14:00:00', end: '2026-03-05T16:50:00',
          extendedProps: { source: 'canvas-class', classId: 'chem_lab', courseName: 'Organic Chemistry Lab', isExam: true },
        }),
      ],
    })
    openCard('Organic Chemistry')
    expect(cards().getByText('Organic Chemistry midterm')).toBeInTheDocument()
    expect(cards().getByText('Organic Chemistry Lab practical')).toBeInTheDocument()
  })
...
describe('ClassesPanel — the day counter', () => {
  ...
  /* An exam is outside the count on both halves. This test used to assert the
     opposite — that an exam kept the day from ever reading complete — which was the
     other implementation of this counter. An exam's `done` is never true, so counting
     it means "1/2" can never become "2/2" however much work you finish; the exam is
     still visible as a chip, a red overdue dot and the workload tint. See the note on
     `dayProgress` in lib/classCalendar.js. */
  it('leaves an exam out of the count, so finished work reads as finished', () => {
    renderPanel({
    ...
    expect(calendar().getByTitle('1 of 1 done')).toBeInTheDocument()
  })
